...
def find_target_neighbors(data, data_std, cost_column="avg_cost_per_marker", county_column="County", target_county="Van Buren", n_neighbors=5, metrics=["wetlandd", "pop_d", "roadoverarea"]):
    ...
    print(f"Step 1: County column '{county_column}' confirmed in data_std.")

    # Filter data_std to only include specified metrics and county names
    try:
        data_for_knn = data_std[[county_column] + metrics].copy()
        print("Step 2: Data prepared for KNN with columns:", data_for_knn.columns)
    except KeyError as e:
        print(f"Error: Missing columns in data_std - {e}")
        return None

    # Find the index of the target county
    try:
        target_index = data_for_knn.index[data_for_knn[county_column] == target_county][0]
        print(f"Step 3: Target index for '{target_county}' found at {target_index}")
    except IndexError as e:
        print(f"Error: Target county '{target_county}' not found in data_for_knn.")
        print("Available counties in data_for_knn:", data_for_knn[county_column].unique())
        return None

    # Prepare data for NearestNeighbors (exclude county column)
    knn_data = data_for_knn.set_index(county_column)[metrics]
    print(f"Step 4: Prepared knn_data with shape {knn_data.shape} for metrics:", metrics)
    ...
    print("Step 5: NearestNeighbors model initialized and fitted.")

    try:
        distances, indices = nbrs.kneighbors(knn_data.loc[[target_county]])
        print(f"Step 6: Nearest neighbors found. Indices: {indices}")
    except KeyError as e:
        print(f"Error: Target county '{target_county}' not found in knn_data. Available counties:", knn_data.index.tolist())
        return None

    # Get the indices of the nearest neighbors (excluding the target county itself)
    neighbor_indices = indices[0][1:]
    print("Step 7: Neighbor indices (excluding target):", neighbor_indices)

    # Map indices back to county names
    neighbor_counties = knn_data.iloc[neighbor_indices].index.tolist()
    print("Step 8: Neighbor counties identified:", neighbor_counties)
    ...
    print("Step 9: Neighbors' data extracted with cost values.")
    ...
    print("Step 10: Target county data added to neighbors data.")
